...
### 2. 실무 적용 시 ⚠️최고 중요⚠️ 주의 사항

Ehcache를 실무에 도입할 때 가장 많이 실수하고, 이로 인해 장애나 버그가 발생하는 포인트들입니다.

#### 1) 내부 메서드 호출 시 캐시 작동 안 함 (Self-Invocation 문제)
...
#### 3) 힙 메모리(Heap Memory) OutOfMemoryError (OOM) 조심
...
## 2. 개선된 로직 (Java Stream / Lambda 적용)
...
#### 8-5. 캐시 크기와 TTL을 반드시 운영 기준으로 설정
...
#### 5-1. `cache:annotation-driven`을 켰으니 모든 CacheService 호출이 Spring Cache 정책을 탄다고 오해
...
#### 5-2. `@Cacheable`을 붙였는데 내부 호출이라 동작하지 않는 경우
...
#### 5-3. Service가 아닌 Controller 쪽 Context에만 설정한 경우
...
### 6. 현재 코드 기준으로 실제 점검해야 할 것

#### 6-1. `ehCacheManager`와 `ehCache`가 같은 캐시를 보는지 확인
...
#### 6-2. `CommonConstants.CACHE_ALIAS_GOODS_UNIT`와 Ehcache alias 일치 확인
...
#### 6-3. 실제 `cache:annotation-driven`이 필요한지 판단
...
#### 6-4. 애노테이션 기반 캐싱을 쓸 경우 public 메서드인지 확인
...
#### 방향 B. 단건 조회는 `@Cacheable`, 목록 조회는 수동 조합
