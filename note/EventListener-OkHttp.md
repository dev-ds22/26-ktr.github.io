### OkHttp `EventListener` 기반 Socket Connection 추적 + Proxy 설정 예제
...
## 이 경우 EventListener 로그의 `connectStart remote`는 목적지 서버가 아니라 **Proxy 서버 주소**로 보일 수 있습니다. HTTPS 요청이면 OkHttp는 proxy에 `CONNECT` 터널을 만들고 이후 TLS handshake를 수행합니다.

### 6. Proxy 사용 방식 2: 인증 Proxy
...
### 7. Proxy 사용 방식 3: Proxy 완전 미사용 강제
...
## 공식 문서 기준 `proxy(Proxy.NO_PROXY)`는 proxy 사용을 완전히 비활성화하는 방식입니다. ([Square Open Source](https://square.github.io/okhttp/3.x/okhttp/okhttp3/OkHttpClient.Builder.html?utm_source=chatgpt.com "OkHttpClient.Builder (OkHttp 3.14.0 API)"))

### 8. Proxy 사용 방식 4: 목적지별 ProxySelector
...
## 핵심은 `try (Response response = ...)`입니다. 이것이 없으면 `connectionReleased`가 늦거나 발생하지 않고, `CLOSE-WAIT`/FD 누수로 이어질 수 있습니다.
...
## 부하 테스트 중에는 `INFO`로 잠깐 올려도 되지만, 운영 상시는 `DEBUG` 또는 sampling이 안전합니다.
