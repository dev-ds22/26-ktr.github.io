# VS Code + Tomcat + Maven 프로젝트 배포/기동 방법
...
target\bk-fo.war
```
# 15. 방법 A - VS Code에서 WAR를 Tomcat에 배포
가장 직관적인 방법입니다.
VS Code Explorer에서:
```text
target
...
http://localhost:8080/bk-fo/
```
# 17. 방법 B - 플러그인 없이 Maven + Tomcat 직접 배포
...
```
# 21. Maven Tomcat Plugin 방식은 우선 추천하지 않음
인터넷에 다음 예제가 많이 나옵니다.
```xml
<plugin>
...
# 폐쇄망 VS Code + Spring Framework 5.3 Web 프로젝트 권장 확장 구성
...
---
# 3. 추가 설치 1순위: XML Language Support by Red Hat
## 확장
```text
Name : XML
Publisher : Red Hat
...
---
# 4. 추가 설치 2순위: Tomcat Server Connector
...
---
# 5. 추가 설치 3순위: JSP Language Support
...
둘 모두 RSP UI를 사용합니다.
---
# 10. Spring Boot Extension Pack은 설치할 필요 없음
인터넷 검색을 하면 거의 항상:
```text
...
---
# 14. 폐쇄망에서는 의존 Extension을 반드시 함께 반입
온라인 Marketplace에서는:
```text
Community Server Connectors
        ↓
...
```
---
# 21. 프로젝트용 권장 `.vscode/settings.json`
