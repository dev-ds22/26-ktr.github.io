...
### 2. 코드 구조적 개선점 (Refactoring Points)
...
### 3. 실무 적용 시 치명적인 주의점 (Architecture & Production Hazards)
...
##### 5-1. `copyBodyToResponse()`가 `finally`에 없음
...
##### 5-2. `LOGGER.debug`가 꺼져 있어도 비용 발생
...
##### 7-1. `OncePerRequestFilter` 사용 권장
...
#### 2. `ContentCachingRequestWrapper` 동작 방식
...
#### 3. `ContentCachingResponseWrapper` 동작 방식
...
#### 8. Spring 5.3 커머스 프로젝트 기준 권장 정책
...
#### 2. 현재 코드에서 이미지/엑셀 응답이 들어오면 생기는 일
...
#### 7. 더 권장하는 개선안: `OncePerRequestFilter` + `shouldNotFilter`
