...
toc_sticky: true
date: "2026-08-19"
last_modified_at: "2026-08-19 14:04:17 +0900"
mermaid: true
---
### 1. 결론
...
### 3-1-1. `<transport lock-timeout="60000"/>`
...
## 6-1. Distributed와 Replicated의 차이
...
# 8. `locking isolation="READ_COMMITTED"`
...
# 11. 앞서 제공한 JGroups 설정과 완전히 연결해 보면
...
## 11-2. 즉 **JBoss 측에서는 웹 세션 클러스터링을 위한 서버 인프라가 거의 완성된 형태**로 보입니다. ([레드햇 문서][1])

# 12. 현재 `sessionVo`와 연결
...
## 12-1. 는 분산 Web Session 환경에서는 cache state 변경으로 처리되는 핵심 코드입니다.

# 13. 그런데 아직 마지막 조건 하나가 남음
...
[1]: https://docs.redhat.com/en/documentation/red_hat_jboss_enterprise_application_platform/7.4/html/configuration_guide/configuring_high_availability "Chapter 24. Configuring High Availability | Configuration Guide | Red Hat JBoss Enterprise Application Platform | 7.4 | Red Hat Documentation"
[2]: https://access.redhat.com/webassets/avalon/d/red_hat_jboss_enterprise_application_platform/7.4/mgmt_model/subsystem/infinispan/cache-container/replicated-cache/index.html "JBoss EAP 7.4.0.GA Model Reference"
[3]: https://docs.redhat.com/en/documentation/red_hat_jboss_enterprise_application_platform/7.3/html/development_guide/clustering_in_web_applications "Chapter 6. Clustering in Web Applications | Development Guide | Red Hat JBoss Enterprise Application Platform | 7.3 | Red Hat Documentation"
...
# 16. 세션 클러스터링과 중복 로그인 제어의 역할을 분리해야 함
...
# 17. 중요한 원리: 기존 HttpSession을 찾아 죽일 필요가 없음
...
# 21. 중요한 특성: "즉시 물리 삭제"와 "즉시 사용 불가"는 다름
...
# 23. 왜 SESSION_ID만 비교하지 않고 TOKEN을 별도로 쓰는가
...
## 26-1. `USER_ID`가 PK이므로 단건 인덱스 조회가 됩니다.

# 27. 로그아웃 DELETE에서 매우 중요한 조건
...
# 30. Spring 5.3에서는 `OncePerRequestFilter` 권장
...
## 30-1. 를 사용하는 것이 적합합니다. 이 Filter는 한 request dispatch에서 중복 실행되지 않도록 처리하는 Spring의 기반 Filter입니다. ([Home][5])
...
## 31-1. 새로운 세션을 만들어서는 안 되므로 `false`를 사용합니다.
...
## 32-1. 이렇게 하면 Filter 자체는 Servlet Filter Chain에서 실행되면서 내부 Service/Mapper는 Spring DI를 그대로 사용할 수 있습니다.

# 33. 반드시 Filter 제외 URL을 설계
...
## 33-1. 실제 시스템의 public URL 정책에 맞게 White List를 관리해야 합니다.
...
## 35-1. 따라서 이 설계는 **Sticky Session에 의존하지 않습니다.**

# 36. JBoss 설정 변경은 필요한가?
...
# 37. 왜 기존 `web/dist` Infinispan에 LOGIN_TOKEN Map을 직접 넣지 않는가
...
## 37-1. 로 분리하는 것이 안전합니다.

# 38. JBoss Infinispan을 Login Registry로 사용하는 대안도 가능
...
# 41. "5초마다만 확인" 같은 Local Cache는 신중
...
## 42-1. 은 이 측면에서 부담이 거의 없습니다.

# 43. 서버 영향 관점에서 가장 중요한 점
