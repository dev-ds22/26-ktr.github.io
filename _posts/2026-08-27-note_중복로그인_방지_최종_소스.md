---
layout: single
title: "중복로그인_방지_최종_소스"
excerpt: "중복로그인_방지_최종_소스"
categories:
  - "tech"
tags:
  - "tech"
...
toc_sticky: true
date: "2026-08-27"
last_modified_at: "2026-08-27 14:54:17 +0900"
mermaid: true
---
# 1. JBoss 중복로그인 방지 구조 및 운영 가이드
...
```
{% endraw %}
### 1-1-4. 동일 WAS에서 중복로그인 발생 시 상세 시퀀스
{% raw %}
```mermaid
sequenceDiagram
...
### 1-1-16. `EgovSessionCookieUtil.java`
````java
...
### 1-1-22. `jboss-deployment-structure.xml`
````xml
...
### 1-1-27. `login/CommandContext.java`
````java
...
### 1-1-30. 다른 개발자가 혼동하지 않아야 할 점
이 파일의 주석에는 Cluster/WAS 간 명령 실행 구조가 설명되어 있지만, 첨부된 현재 실행 경로는 그 구조를 사용하지 않는다.

### 1-1-31. `login/CustomHttpSessionBindingListener.java`
````java
package bk.comm.login;
...
### 1-1-37. `login/CustomHttpSessionIdListener.java`
````java
package bk.comm.login;
...
### 1-1-41. `login/exception/ClusterLoginControlException.java`
````java
...
### 1-1-44. `login/LocalLoginPreventer.java`
````java
package bk.comm.login;
...
### 1-1-49. `login/LoginGeneration.java`
````java
package bk.comm.login;
...
### 1-1-54. `login/LoginPreventer.java`
````java
package bk.comm.login;
...
### 1-1-58. `login/LoginSessionManager.java`
````java
package bk.comm.login;
...
### 1-1-69. `login/SessionCommand.java`
````java
package bk.comm.login;
