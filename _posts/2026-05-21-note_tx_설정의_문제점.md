...
toc_sticky: true
date: "2026-05-21"
last_modified_at: "2026-05-21 11:26:02 +0900"
---
정확도: 96%
## 1. 결론
...
  → OrderHistoryService.insertOrderHistory()
```
...
|        실무 판단        | 가장 일반적이고 안전                                   |
  ...
  - 흐름 : 
	  - Controller
		...
		→ OrderHistoryService.insertOrderHistory() : REQUIRED, 기존 트랜잭션 참여
		→ 정상 종료
		→ A + B 작업 함께 commit  
...
```java
OrderService.saveOrder()
  → OrderHistoryService.selectOrderHistory()
```
...
- 흐름:
	- Controller
		→ OrderService.saveOrder() : REQUIRED, 새 트랜잭션 시작
		→ OrderHistoryService.selectOrderHistory() : SUPPORTS, 기존 트랜잭션 참여
...
```text
Controller
→ OrderService.selectOrder() : SUPPORTS, 트랜잭션 없음
→ OrderHistoryService.insertOrderHistory() : REQUIRED, 새 트랜잭션 시작
→ OrderHistoryService 종료 시 commit
→ OrderService 나머지 로직 실패
```

- 이 경우 `OrderHistoryService.insertOrderHistory()`는 이미 commit될 수 있습니다. 이후 OrderService의 후속 로직에서 예외가 발생해도 OrderService 자체는 트랜잭션이 없었기 때문에 OrderHistoryService에서 commit된 내용을 같이 rollback할 수 없습니다.

- 예:
```java
public void selectOrder() {
    historyService.insertOrderHistory(); // REQUIRED로 별도 트랜잭션 시작 후 commit 가능
...
결과:
```text
  OrderHistoryService.insertOrderHistory() commit됨
...
```text
  ...
  → OrderService.selectOrder() : SUPPORTS, 기존 트랜잭션 참여
  → OrderHistoryService.insertOrderHistory() : REQUIRED, 기존 트랜잭션 참여
```
...
```text
  Controller
  → OrderService.selectOrder() : SUPPORTS, 트랜잭션 없음
  → OrderHistoryService.selectOrderHistory() : SUPPORTS, 트랜잭션 없음
```
...
    historyService.insertOrderHistory();
    // 후속 로직
}
```

`processOrder()`가 `save*`, `insert*`, `update*`, `delete*`에 걸리지 않으면:

```text
OrderService.processOrder() → SUPPORTS read-only=true
OrderHistoryService.insertOrderHistory() → REQUIRED
```
...
- 예:
```text
OrderService.selectOrder() : REQUIRED read-only=true
→ OrderHistoryService.insertOrderHistory() : REQUIRED read-write
```
...
    public OrderService(OrderHistoryService historyService) {
...
OrderHistoryService.insertOrderHistory()는 같은 트랜잭션 참여
A 또는 B에서 예외 발생
전체 rollback
```
## 12. 문제 있는 구조
...
- `saveOrder()`에서 같은 클래스의 `updateOrder()`를 호출하면 프록시를 통과하지 않을 수 있습니다. 이 경우 `updateOrder()`에 해당하는 트랜잭션 속성이 별도로 적용되지 않을 수 있습니다. Spring 공식 문서도 프록시 모드에서는 self-invocation이 트랜잭션으로 이어지지 않는다고 설명합니다. ([Home][1])
