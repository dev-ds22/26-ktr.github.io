...
toc_sticky: true
date: "2026-06-10"
last_modified_at: "2026-06-10 11:44:27 +0900"
mermaid: true
---
## 1. 결론
...
## 2. DBCP `testWhileIdle=true` 관련 옵션과 Evictor 관련 옵션 정리
...
## 3. `testWhileIdle=true`와 직접 관계있는 옵션
...
## 4. `testWhileIdle=true`와 관련 있지만, 혼동하면 안 되는 옵션
...
## 5. `testWhileIdle`과 무관하게 Evictor 자체와 관계있는 옵션

### 5-1. Evictor 실행/검사 범위 옵션
...
### 5-2. Evictor와 abandoned Connection 회수 관련 옵션
...
### 7-1. `testWhileIdle=true`를 켰을 때 반드시 같이 봐야 하는 설정
...
### 7-2. `testWhileIdle`과 무관하지만 Evictor에 중요한 설정
...
## 15. `numTestsPerEvictionRun`에 따른 실제 적용 범위
...
## 22. Service가 오류를 반환하는 경우

### 22-1. `testOnBorrow=false`인 경우
...
## 23. `testWhileIdle=true`만으로 충분하지 않은 이유
...
## 24. Spring `@Transactional` 환경에서의 실제 오류 흐름
...
### 24-1. 일반적인 `@Transactional REQUIRED` 흐름
