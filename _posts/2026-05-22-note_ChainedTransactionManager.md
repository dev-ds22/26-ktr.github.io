---
layout: single
title: "ChainedTransactionManager"
excerpt: "ChainedTransactionManager"
categories:
  - "tech"
tags:
  - "tech"
...
toc_sticky: true
date: "2026-05-22"
last_modified_at: "2026-05-22 17:10:28 +0900"
---
## 1. ChainedTransactionManager에 txManager, txManagerMail 등록
```xml
...
### 1-1. `ChainedTransactionManager`의 역할
...
### 1-2. `txManager`, `txManagerMail`을 list에 등록하면 같은 Transaction인가?
...
### 1-3. 중요한 오해: list에 등록만 하면 자동 적용되는가?
...
### 1-6. `txManager`, `txManagerMail` 구성 시 판단 기준
...
#### 1-8-2. 권장 2: 메인 DB commit 후 메일 이력 별도 저장
...
### 2-3. 현재 `transactionManager`의 실제 의미
...
## 3. `ChainedTransactionManager`는 delegate transaction manager를 등록 순서대로 시작하고, commit/rollback은 역순으로 수행합니다. commit 중 실패가 발생하면 아직 commit되지 않은 transaction manager는 rollback됩니다. ([Home](https://docs.spring.io/spring-data/commons/docs/current/api/org/springframework/data/transaction/ChainedTransactionManager.html?utm_source=chatgpt.com "ChainedTransactionManager (Spring Data Core 4.0.5 API)"))

### 3-1. 가장 큰 실무 문제

#### 3-1-1. 모든 Service 호출 시 Main DB + Mail DB 트랜잭션이 같이 시작될 가능성
...
#### 3-1-2. 두 DB가 완전한 하나의 트랜잭션으로 묶이지 않음
...
#### 3-1-3. Mail DB 장애가 전체 서비스 장애로 번질 수 있음
...
#### 3-1-4. 모든 메소드 `rollback-for="Exception"`의 영향
...
#### 3-1-5. `REQUIRED` 내부 rollback-only 문제
...
## 5. 다만 너무 넓게 잡으면 불필요한 bean까지 트랜잭션 대상이 될 수 있으므로 주의해야 합니다.

#### 5-1-1. `*Service`만 매칭하므로 `*ServiceImpl` 누락 가능
...
#### 5-1-2. XML `aop:config`가 두 번 분리되어 있음
...
## 6. 단, 이 개선은 구조 정리일 뿐이고, 핵심 문제인 `ChainedTransactionManager` 일괄 적용 문제는 별도로 해결해야 합니다.

### 6-1. 현재 설정에서 발생 가능한 대표 오류

#### 6-1-1. Mail DB 장애로 Main 업무 실패
...
#### 6-1-3. 조회 API에서 connection pool 고갈
...
#### 6-1-4. 내부 예외 catch 후 최종 rollback
...
#### 6-1-5. self-invocation으로 트랜잭션 미적용
...
## 7. Spring AOP proxy 기반 트랜잭션은 일반적으로 proxy를 통해 외부에서 호출될 때 적용됩니다. 같은 클래스 내부 호출은 proxy를 거치지 않으므로 트랜잭션이 기대와 다르게 동작할 수 있습니다.

### 7-1. 개선 방향

#### 7-1-1. 개선안 1. Main DB 기본 트랜잭션과 Mail DB 트랜잭션을 분리
...
## 8. 단, 위 pointcut의 `aa..mail..service..*Service.*(..)`는 예시입니다. 실제 패키지가 `aa.app.mail.service`, `aa.api.mail.service`, `aa.mail.service` 중 무엇인지에 맞춰 조정해야 합니다.

#### 8-1-1. 개선안 2. `ChainedTransactionManager` 기본 적용 제거
...
#### 8-1-2. 개선안 3. 메일 발송/메일 이력은 Outbox 구조 검토
...
## 9. 다만 XML 속성 안에서는 줄바꿈/공백 문제를 피하기 위해 한 줄로 관리하는 편이 안전합니다.
...
## 10. 주의: `mailServiceTx` pointcut은 실제 패키지 구조 확인 후 조정해야 합니다. 또한 Mail Service가 Main Service pointcut에도 동시에 걸리면 advice가 중복 적용될 수 있으므로 패키지 분리 또는 exclusion이 필요합니다.

### 10-1. 최소 변경 개선안

현재 운영 중이고 큰 구조 변경이 어렵다면, 최소 변경은 다음 순서가 안전합니다.

#### 10-1-1. 1단계: 기본 transaction manager를 Main DB로 변경
...
#### 10-1-2. 2단계: Mail Service 전용 advice 추가
...
#### 10-1-3. 3단계: Mail Service pointcut만 별도 지정
...
## 11. log4j/log4j2 XML 환경이면 동일 패키지를 DEBUG로 조정하면 됩니다.
...
## 12. Deadlock, wait connection 발생

### 12-1. 현재 Transaction 설정으로 Deadlock 발생 가능성
...
### 13-1. 현재 설정에서 Deadlock 가능성을 높이는 지점

#### 13-1-1. 모든 Service가 두 DB 트랜잭션을 동시에 시작
...
### 13-2. 발생 가능한 Deadlock 시나리오

#### 13-2-1. Main DB 내부 Deadlock
...
## 14. 이 경우 원인은 `ChainedTransactionManager` 자체라기보다 **SQL 실행 순서 불일치**입니다. 다만 모든 Service가 `REQUIRED`로 길게 묶이면 lock 보유 시간이 길어져 deadlock 확률이 올라갑니다.

#### 14-1-1. Main DB와 Mail DB를 함께 쓰는 교차 Deadlock 또는 Lock Wait
...
## 15. 이 경우 두 DB가 같은 MariaDB 인스턴스 안에 있으면 InnoDB가 deadlock으로 감지할 수 있습니다. 반대로 Main DB와 Mail DB가 서로 다른 DB 서버라면 각 DB는 전체 대기 관계를 알 수 없으므로 deadlock으로 감지되지 않고 `lock wait timeout` 형태로 나타날 수 있습니다.
...
#### 15-1-1. 조회 메소드까지 REQUIRED로 묶여 Lock 유지 시간이 증가
...
#### 15-1-2. Connection Pool 고갈이 Deadlock처럼 보일 수 있음
...
## 16. 이것은 DB deadlock은 아니지만, 운영 현장에서는 “서버가 멈춘 것처럼 보이는 현상”으로 나타날 수 있습니다.

### 16-1. 현재 설정에서 Deadlock 위험도를 높이는 원인 정리
...
#### 16-2-1. 가장 우선: `ChainedTransactionManager`를 전역 기본값에서 제거
...
#### 16-2-2. Mail DB는 Mail Service에만 별도 적용
...
#### 16-2-3. 두 DB를 반드시 함께 갱신해야 한다면 갱신 순서 고정
...
## 17. 즉, 가능하면 두 DB를 한 트랜잭션처럼 묶지 않는 방향이 더 안전합니다.

#### 17-1-1. 외부 API, 메일 발송, 파일 처리는 트랜잭션 밖으로 분리
...
## 18. 이 방식은 DB lock 보유 시간을 줄이고, 외부 시스템 장애가 DB 트랜잭션을 붙잡는 문제를 줄입니다.

#### 18-1-1. 조회 메소드는 `SUPPORTS + read-only`로 분리
...
## 19. Main DB 업무인데 Mail DB pool active가 같이 증가하면 현재 설정 문제가 실제로 영향을 주고 있는 것입니다.
...
## 22. 즉, **Main DB만 사용하는 업무라도 Mail DB connection 획득 단계에서 막힐 수 있습니다.**

### 22-1. 장애 발생 가능성

#### 22-1-1. Mail DB pool 10개가 전체 서비스 병목이 됨
...
## 23. DB deadlock이 아니라 **connection pool wait**인데, 현장에서는 WAS가 멈춘 것처럼 보일 수 있습니다.

#### 23-1-1. Main DB connection까지 같이 묶여 낭비됨
...
#### 23-1-2. 동일 DB 서버라서 DB 전체 connection 한도에도 영향
...
## 24. 동일 서버의 다른 스키마라고 해도 DB connection 수는 서버 단위로 소비된다고 보는 것이 안전합니다.

#### 24-1-1. Deadlock보다 Lock Wait/Pool Timeout 가능성이 더 큼
...
#### 24-2-1. 시나리오 A: Main 업무 폭주로 Mail pool 고갈
...
#### 24-2-2. 시나리오 B: Mail DB 지연으로 Main 업무 장애
...
#### 24-2-3. 시나리오 C: 다중 WAS에서 DB connection 총량 초과
...
### 24-3. 문제 회피 1순위: 전역 `ChainedTransactionManager` 제거
...
### 24-5. 문제 회피 3순위: 부득이하게 Chained를 유지할 경우

구조 변경이 어렵다면 최소한 아래 조치를 권장합니다.

#### 24-5-1. Chained 적용 범위 축소
...
#### 24-5-2. Mail pool을 무작정 100으로 올리지 않기
...
## 25. 따라서 순서 변경은 임시 완화책일 뿐이고, 근본 해결은 전역 Chained 제거입니다.

### 25-1. DBCP2 설정 측면 개선안

#### 25-1-1. Main/Mail pool을 업무 특성에 맞게 분리
...
#### 25-1-2. `maxWaitMillis`를 무한 또는 과도하게 길게 두지 않기
...
## 26. Main 업무 트래픽 증가 시 `dataSourceMail.numActive`가 같이 증가하면 현재 구조 문제가 실제로 발생 중인 것입니다.

### 26-1. 점검 SQL/명령

#### 26-1-1. DB 서버 connection 현황
...
### 27-1. 현재 설정 기준 TransactionManager 매핑 정리

#### 27-1-1. Bean ID 기준 매핑
...
#### 27-1-3. 실제 Service에 적용되는 TransactionManager
...
## 28. 별도의 TransactionManager 생성 고려 시
...
## 29. 이 구조에서는 Main DB만 사용하는 업무도 Mail DB pool 상태에 영향을 받을 수 있습니다.
...
## 30. 따라서 “새로 생성”보다는 **기존 `txManager`를 Main 업무 기본 transaction manager로 사용**하는 것이 더 적절합니다.

### 30-1. 적용 예시

#### 30-1-1. Chained는 이름을 명확히 변경
...
## 31. 다만 `name="*"`를 Chained 전용 advice에 넣으면 pointcut이 넓을 때 다시 모든 메소드가 Chained를 탈 수 있으므로, Chained advice는 가능하면 메소드명을 제한하는 것이 안전합니다.

### 31-1. AOP pointcut 분리 예시
...
## 32. 하지만 위 구조에는 중요한 문제가 있습니다. `*MailLinkedService`가 `mainServiceTx`에도 같이 걸릴 수 있습니다. 그러면 **동일 메소드에 Main advice와 Chained advice가 중복 적용**될 수 있습니다.

### 32-1. 가장 중요한 주의점: Pointcut 중복 금지
...
#### 32-4-1. Main transaction 안에서 Chained Service를 호출하는 경우
...
#### 32-4-2. Chained transaction 안에서 Main Service를 호출하는 경우
...
#### 32-4-3. Main Service에서 Mail Service를 별도 txManagerMail로 호출하는 경우
...
## 33. 따라서 Mail 처리는 가능하면 직접 호출하지 말고 Outbox 또는 after-commit 이벤트 구조가 더 안전합니다.
