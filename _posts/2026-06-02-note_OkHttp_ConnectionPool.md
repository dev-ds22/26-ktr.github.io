---
layout: single
title: "OkHttp_ConnectionPool"
excerpt: "OkHttp_ConnectionPool"
categories:
  - "tech"
tags:
  - "tech"
...
## 3. OkHttp 공식 Builder 문서에서 `connectTimeout`은 TCP socket 연결을 맺는 시간에 적용되며 기본값은 10초로 설명됩니다. `readTimeout`도 기본값 10초로 설명됩니다. ([Square Open Source](https://square.github.io/okhttp/3.x/okhttp/okhttp3/OkHttpClient.Builder.html?utm_source=chatgpt.com "OkHttpClient.Builder (OkHttp 3.14.0 API)"))

### 3-1. ConnectionPool과 Timeout의 역할 구분
...
## 4. `ConnectionPool`은 idle connection 보관 정책이고, `Dispatcher`는 요청 실행 동시성 제어에 가깝습니다. 둘을 혼동하면 `maxIdleConnections`를 동시 요청 제한값처럼 잘못 설정할 수 있습니다.

### 4-1. 실무 권장 설정 예시

#### 4-1-1. 일반적인 내부 API 호출
...
#### 4-1-2. L4 idle timeout이 180초인 경우
...
### 4-3. Response close와 Pool 반환 관계
...
## 5. 중요한 점은 OkHttp의 socket pool은 DB Pool처럼 `maxTotal=100` 식으로 전체 connection 수를 강하게 제한하는 구조가 아니라는 점입니다.

### 5-1. Spring 5.3/JDK11 기준 Bean 구성 권장안
